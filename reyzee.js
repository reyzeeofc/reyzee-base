...
async function start() {
   ...
   store.bind(reyzee.ev)

   // push update name to store.contacts
   reyzee.ev.on("contacts.update", (update) => {
   ...
   // bind extra client
   await Client({ reyzee, store })

   // login use pairing code
   // source code https://github.com/WhiskeySockets/Baileys/blob/master/Example/example.ts#L61
   if (pairingCode && !reyzee.authState.creds.registered) {
      ...
      setTimeout(async () => {
         let code = await reyzee.requestPairingCode(phoneNumber)
   ...
   if (useMobile && !reyzee.authState.creds.registered) {
      const { registration } = reyzee.authState.creds || { registration: {} }
      ...
      async function enterCode() {
         try {
            ...
            const response = await reyzee.register(code.replace(/[^0-9]/g, '').trim().toLowerCase())
      ...
      async function askOTP() {
         ...
         try {
            await reyzee.requestRegistrationCode(registration)
            await enterCode()
         } catch (e) {
   ...
   reyzee.ev.on("connection.update", async (update) => {
      ...
      if (connection === "open") {
         reyzee.sendMessage(config.options.owner[0] + "@s.whatsapp.net", {
            text: `${reyzee?.user?.name || "Reyzee"} has Connected...`,
         })
      }
   })

   // write session
   reyzee.ev.on("creds.update", saveCreds)

   // messages
   reyzee.ev.on("messages.upsert", async (message) => {
      if (!message.messages) return
      const m = await Serialize(reyzee, message.messages[0])
      await (await import(`./event/message.js?v=${Date.now()}`)).default(reyzee, m, message)
   })

   // group participants update
   reyzee.ev.on("group-participants.update", async (message) => {
      await (await import(`./event/group-participants.js?v=${Date.now()}`)).default(reyzee, message)
   })

   // group update
   reyzee.ev.on("groups.update", async (update) => {
      await (await import(`./event/group-update.js?v=${Date.now()}`)).default(reyzee, update)
   })

   // auto reject call when user call
   reyzee.ev.on("call", async (json) => {
      if (config.options.antiCall) {
         for (const id of json) {
            if (id.status === "offer") {
               let msg = await reyzee.sendMessage(id.from, {
               ...
               reyzee.sendContact(id.from, config.options.owner, msg)
               await reyzee.rejectCall(id.id, id.from)
            }
         }
      }
   })
